...
export default {
    ...
    methods: {
        ...
        async AjaxReceiptTemplate(student_name,amount_in_words,date,amount_in_numbers,index_number){
            try{
                let receipt_id = this.$route.params.id

                let receipt_res = await axios({
                    url: `http://agile-brook-14477.herokuapp.com/api/v1/receipt_template/update?id=${receipt_id}`,
                    method: "PUT",
                ...
                if(receipt_res.data.res === "updated"){
                    this.loading = false
                    this.res = "updated"
...
this.$router.push({name: 'ShowReceipts'})
   ...
   async created(){
        ...
        if(!isNaN(route_id)){
            try {
                        let show_receipt = await axios({
                        url: `http://agile-brook-14477.herokuapp.com/api/v1/receipt_template/show?id=${route_id}`,
                        method:"GET"
                    })
                    if(show_receipt.data.res === "found"){
                        this.student_name = show_receipt.data.data.student_name
                        this.amount_in_words = show_receipt.data.data.amount_in_words
                        this.amount_in_numbers = show_receipt.data.data.amount_in_numbers
                        this.index_number = show_receipt.data.data.index_number
                        this.date =show_receipt.data.data.date
                        console.log(show_receipt.data)
                    }
            } catch(e) {
                // statements
                console.log(e);
